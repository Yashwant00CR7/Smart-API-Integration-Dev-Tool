...
The engine uses **LangGraph** to coordinate a self-correcting loop that feeds compiler and test logs back to the LLM (up to 3 retries) until the generated assets compile and pass mock checks.
...
* **AI Generation**: Google Gemini API (via official `google-genai` SDK) & local Ollama (`qwen2.5-coder`)
...
├── src/
│   ├── app.py              # FastAPI server routes, static assets mapping, and CORS config
│   ├── config.py           # Configuration manager validating env parameters via Pydantic
│   ├── agent.py            # LangGraph StateGraph, code generator, and execution nodes
│   └── services/
...
```text
  [Docs Scraping] ────► [Initial Generator Node] ────► [Execution Sandbox Node]
                                ▲                              │
                                │ (Test Fails & Retries < 3)   │
                                └──────────────────────────────┴─► [Test Passes / Ends]
                                     Heal Code with Error Logs
```
...
2. **Predict (Generator Node)**: The model outputs structured JSON matching the Pydantic schema containing wrapper code, README, endpoints list, and a test suite.
3. **Act & Verify (Executor Node)**: The Python executor creates an isolated UUID directory under `temp/run_{uuid}`, writes code files, and triggers the language's native test runner (e.g. `pytest`, `go test`, `javac`).
4. **Self-Heal (Loop)**: If the test returns a non-zero exit code, the logs (stdout/stderr) are saved to the state, and the graph loops back to the generator, instructing the model to repair the code. If tests pass, it terminates immediately and packages the clean deliverables.
